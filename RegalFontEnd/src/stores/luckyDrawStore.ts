...
export const useLuckyDrawStore = defineStore('luckyDrawStore', {
    ...
    actions: {
        async fetchPagedLuckyDraws() {
            ...
            try {
                ...
                if (result?.succeeded === true) {
                    this.luckyDraws = result.data.items;
        ...
        async fetchPagedCustomerRewards() {
            ...
            this.loading = true;
            try {
                const result = await luckyDrawService.fetchPagedCustomerRewards(this.customerRewardQuery);
                if (result?.succeeded === true) {
        ...
        async fetchPagedRewards() {
            ...
            this.loading = true;
            try {
                const result = await luckyDrawService.fetchPagedRewards(this.rewardQuery);
                if (result?.succeeded === true) {
        ...
        async confirmReceiveCustomerReward(id: string, note?: string) {
            return await serviceFactory.luckyDrawService.confirmReceiveCustomerReward(id, note);
        },

        async confirmAcceptanceCustomerReward(id: string, note?: string) {
            return await serviceFactory.luckyDrawService.confirmAcceptanceCustomerReward(id, note);
        },
