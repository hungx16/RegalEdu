<template>
    <div>
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h5 class="fw-bold mb-0">{{ t('student.contactListTitle') }}</h5>
            <el-button v-if="!isView" type="primary" icon="el-icon-plus" @click="emit('addContact')" circle />
        </div>
        ...
        <div v-for="(contact, index) in contacts" :key="index" class="contact-card p-4 mb-3 rounded-3 shadow-sm border">
            ...
            <el-row :gutter="20">
                <el-col :span="6">
                    <span class="text-body-secondary d-block">{{ t('student.contactName') }}</span>
                    ...
                    <span class="text-body-secondary d-block mt-2">{{ t('student.contactEmail') }}</span>
                    <el-form-item>
                ...
                <el-col :span="6">
                    <span class="text-body-secondary d-block">{{ t('student.contactRelationship') }}</span>
                    ...
                    <span class="text-body-secondary d-block mt-2">{{ t('student.username') }}</span>
                    <el-form-item>
                ...
                <el-col :span="10">
                    <span class="text-body-secondary d-block">{{ t('student.contactPhone') }}</span>
                    <el-form-item :prop="'contacts.' + index + '.phone'" :rules="contactRules.phone">
                        <el-input v-model="contacts[index].phone" :disabled="isView"
                            :placeholder="t('student.contactPhonePlaceholder')" />
                    </el-form-item>
                    <span class="text-body-secondary d-block mt-2">{{ t('student.contactAddress') }}</span>
                    <el-form-item>
                ...
                <el-col :span="2" v-if="!isView" class="d-flex justify-content-end align-items-start">
                    <el-button @click="removeContact(index)" type="danger" icon="el-icon-delete" circle />
                </el-col>
            </el-row>
        </div>

        <div v-if="contacts.length === 0 && isView" class="text-center text-body-secondary mt-5">
            {{ t('student.noContactsFound') }}
        </div>
    </div>
</template>
