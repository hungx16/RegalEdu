<template>
    <BaseDialogForm ref="baseDialogRef" :visible="visible" :title="modeTitle" :form-data="formData" :rules="rules"
        ...
        <template #form>
            <el-row :gutter="20">
                <el-col :span="12">
                    <label class="required fs-6 fw-semibold mb-2 d-block">{{ t('promotionGroup.name') }}</label>
                    <el-form-item prop="name">
                        <el-input v-model="formData.name" :disabled="isView"
                            :placeholder="t('promotionGroup.namePlaceholder')" />
                    </el-form-item>
                </el-col>
                ...
                <el-col :span="24">
                    <label class="fs-6 fw-semibold mb-2 d-block">{{ t('promotionGroup.description') }}</label>
                    <el-form-item prop="description">
                        <el-input v-model="formData.description" type="textarea" :rows="3" :disabled="isView"
                            :placeholder="t('promotionGroup.descriptionPlaceholder')" />
                    </el-form-item>
                </el-col>
            </el-row>
        </template>
...
import type { PromotionGroupModel } from '@/api/PromotionGroupApi';
...
const modeTitle = computed(() => {
    if (props.mode === 'create') return t('promotionGroup.add');
    if (props.mode === 'edit') return t('promotionGroup.edit');
    return t('promotionGroup.view');
});

const defaultFormData: Partial<PromotionGroupModel> = {
    name: '',
    description: '',
...
watch(
    () => groupStore.selectedPromotionGroup,
    (data) => {
        if (data && data.id) {
            formData.value = { ...data };
        } else {
