    ...
    <div class="card-body pt-0">
      ...
      <div class="table-responsive">
        <table class="table align-middle gs-0 gy-3">
          ...
          <tbody>
            <tr
              v-for="(item, index) in store.luckyDraws"
              :key="item.id || index"
            >
              <td>{{ index + 1 }}</td>
...
const activeCount = computed(
  () => (store.luckyDraws || []).filter((x) => x.status === 1).length,
);
const completedCount = computed(
  () => (store.luckyDraws || []).filter((x) => x.status === 0).length,
);
...
onMounted(async () => {
  ...
  console.log("store after load", store);
  console.log("state after load", store.$state);
  console.log("luckyDraws after load", store.luckyDraws);
});
</script>
