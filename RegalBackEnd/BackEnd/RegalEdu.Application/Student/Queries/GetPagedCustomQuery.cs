...
namespace RegalEdu.Application.Student.Queries
{
    ...
    public class GetPagedCustomQuery : IRequest<Result<PagedResult<StudentModel>>>
    {
        public required CustomQuery StudentQuery { get; set; }
    }

    public class GetPagedCustomQueryHandler : IRequestHandler<GetPagedCustomQuery, Result<PagedResult<StudentModel>>>
    {
        private readonly IRegalEducationDbContext _db;
        private readonly PagingOptions _paging;

        public GetPagedCustomQueryHandler(IRegalEducationDbContext db, PagingOptions paging)
        {
            _db = db; _paging = paging;
        }

        public async Task<Result<PagedResult<StudentModel>>> Handle(GetPagedCustomQuery request, CancellationToken ct)
        {
            var q = request.StudentQuery;
            var query = _db.Students.AsNoTracking().Where(s => s.StudentStatus==0);
